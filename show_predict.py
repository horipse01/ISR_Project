...
def draw_predicted_path(video_img, pred_track_result):
    batchsize = pred_track_result.shape[0]
    grid_size = pred_track_result.shape[1]  # 16 (16x16 grid)
    time_steps = pred_track_result.shape[3]  # 16 (time steps)

    # 使用不同颜色绘制每个时间步的路径
    color_map = plt.cm.get_cmap('hsv', time_steps)  # 使用HSV色谱来生成不同的颜色

    # 将路径信息从pred_track_result提取出来
    for b in range(batchsize):
        # 处理每个batch
        for t in range(time_steps):
            path = pred_track_result[b, :, :, t, :]  # 16x16的路径，每个点包含(y, x)
            # 获取颜色：根据时间步t选择颜色
            color = color_map(t / time_steps)  # 获得当前时间步的颜色

            # 将路径中的(y, x)坐标转化为像素坐标系中的位置
            for i in range(grid_size):
                for j in range(grid_size):
                    y, x = path[i, j]
                    # 转换成图像坐标（假设[0, 1]归一化到[0, img_size]，比如[0, 512]）
                    img_height, img_width, _ = video_img.shape
                    pixel_y = int(y * img_height)
                    pixel_x = int(x * img_width)

                    # 使用颜色绘制路径点
                    # `color` 是RGBA格式，转换为BGR格式
                    bgr_color = (int(color[2] * 255), int(color[1] * 255), int(color[0] * 255))
                    cv2.circle(video_img, (pixel_x, pixel_y), radius=2, color=bgr_color, thickness=-1)
...
def rollout(policy, env, horizon, render=False, video_writer=None, video_skip=5, return_obs=False, camera_names=None, real=False, rate_measure=None):
    ...
    try:
        for step_i in range(horizon):
            ...
            y_coords = torch.linspace(0, 1, grid_size).view(-1, 1)  # (16, 1)
            x_coords = torch.linspace(0, 1, grid_size).view(1, -1)  # (1, 16)
            y_grid, x_grid = torch.meshgrid(y_coords.squeeze(), x_coords.squeeze())

            coords = torch.stack((y_grid, x_grid), dim=-1)  
            coords = coords.unsqueeze(0).repeat(batchsize, 1, 1, 1)

            pred_track_result = policy.pred_track(query_points=coords, ob=obs)
            ...
            if video_writer is not None:
                if video_count % video_skip == 0:
                    ...
                    video_img = draw_predicted_path(video_img, pred_track_result)
